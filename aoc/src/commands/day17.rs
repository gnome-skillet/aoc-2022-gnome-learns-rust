...
impl RockPile {
    fn new() -> RockPile {
        ...
        RockPile { rocks, offset: 0, total_rocks: 0 }
    }
    ...
    fn top_of_pile(&self) -> Option<String> {
        // If the chamber is less than 8 levels tall, we can't take a skyline
        if self.nlayers() < 60 {
            return None;
        }

        let part = Vec::from_iter(self.rocks.iter().rev().take(60).cloned());
        // Take the top 8 levels of the chamber and fold them into a 128-bit integer.
        let result = String::from_utf8(part).ok()?;
            //self.rocks.iter().rev().take(32).fold(U256::MIN, |acc, byte| (acc << 32) | U256::from(*byte));
        Some(result)
    }
    ...
    fn display(&self) {
        ...
        let string = string.replace("0", ".");
        let total_rocks: usize = self.total_rocks;
        let height: usize = self.nlayers();
        let msg =
            format!("After {total_rocks} rocks, the tower of rocks will be {height} units tall");
        println!("{}\n{}", msg, string);
    }
    ...
    fn add_rocks_to_pile(&mut self, jets: Vec<JetDirection>, total_rocks: usize) {
        ...
        let mut visited = HashMap::with_capacity(3066);
        for shape in rocks {
                ...
                if self.is_blocked_below(&rock) {
                    ...
                    if let Some(top) = self.top_of_pile() {
                        ...
                        if let Some((prev_rocks, prev_height)) = visited.get(&stage) {
                            let rocks_in_cycle: usize = self.total_rocks - prev_rocks;
                            let cycle_height: usize = self.height() - prev_height;
                            let ncycles: usize =
                                    (total_rocks - self.total_rocks) / rocks_in_cycle;

                            self.total_rocks += rocks_in_cycle * ncycles;
                            self.offset += ncycles * (cycle_height);

                            visited.clear();
                        } else {
                            visited.insert(stage, (self.total_rocks, self.height()));
                        }
                    }
                    break;
                } else {
            ...
            if self.total_rocks == total_rocks {
                break;
            }
        }
    }
